# Created by Anthony Siragusa
# Version 1.0
# Simple discord bot that pulls summoner rank & recent match data
# Formats the data with discord embeds and sends resulting message in chat
...
#Rank Icon links for discord image embed
...
#Function to determine what Rank Icon link to grab
...
lol_watcher = LolWatcher('YOUR RIOT GAMES API GOES HERE')
NA = 'NA1'
...
async def opgg(ctx, *, summonerName):
    ...
    #If summoner name contains space, replace spaces with %20 (otherwise link to op.gg doesn't work)
    if ' ' in ign:
        ign = ign.replace(" ", "%20")
    ...
    #Fetches summoner's last 5 ranked games
    ...
    for match in match_list:
        ...
        #Looks through json data to find the correct player's game data
        #Once found fetches player's KDA, Game Result, and Champion and creates an embed with the info
